...
describe ThirteenthAge::Monster::Huge do
  let(:monster) { ThirteenthAge::Monster::Huge.new(level: 5) }

  it 'gets instantiated with a level' do
    expect {ThirteenthAge::Monster::Huge.new(level: 1)}.to_not raise_error
  end
  ...
  it 'returns the strike_damage' do
    expect(monster.strike_damage).to eq(54)
  end

  it 'returns the hp' do
    expect(monster.hp).to eq(216)
  end
